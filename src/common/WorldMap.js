...
let deviceWidth = window.innerWidth - 32;
...
class WorldMap extends React.Component {
  constructor(props) {
    ...
    this.state = {
      deviceWidth,
      actualWidth,
      width: deviceWidth > 600 ? deviceWidth * 0.7 : deviceWidth,
      height: deviceWidth > 600 ? deviceWidth / 2 : deviceWidth,
    };
  }

  drawWorldMap = () => {
    const { width, height, deviceWidth, actualWidth } = this.state;

    var projectionScale = (origProjectionScale =
      actualWidth < 1200 ? height / 2.1 : height / 3);
    // var projectionScale = (origProjectionScale = height / 4.4);
    // var translation = [width / 2, height / 2];
    var translation =
      actualWidth < 1200 ? [width / 2, height / 2] : [width / 2, height / 3];
  ...
  handleResize = () => {
    ...
    let actualWidth = window.innerWidth;
    this.setState({
      width: actualWidth > 600 ? deviceWidth * 0.7 : deviceWidth,
      height: actualWidth > 600 ? deviceWidth / 2 : deviceWidth,
    });
  };
